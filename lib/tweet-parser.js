...
export function extractTweets(endpoint, data) {
  ...
  // Only process it if the tweet has article data — regular tweets from this
  // endpoint are already captured via timeline endpoints.
  if (endpoint === 'TweetResultByRestId') {
    const result = data?.data?.tweetResult?.result;
    if (!result) return [];
    const raw = unwrapTweetResult(result);
    if (!raw || !raw.article?.article_results?.result) return [];
